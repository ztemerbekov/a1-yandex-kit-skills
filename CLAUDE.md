...
(`specs/kit-swagger.openapi.json` — 161 operations, 24 tag groups, the single source of truth).
...
npm test            # unit tests, no network (currently 393: core 37 + codegen 3 + mcp 318 + setup 35)
...
- `packages/mcp` — `mcp-yandex-kit`: stdio MCP server with 71 tools; `src/tools/*.ts` is
  one file per domain; `meta.ts` hosts the trio (`search_operations`,
  `get_operation_schema`, `kit_request`) that covers all 161 operations (75 have curated tools).
