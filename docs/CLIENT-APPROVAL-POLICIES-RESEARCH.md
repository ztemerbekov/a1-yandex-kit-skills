...
> **Решение после исследования (2026-08-16).** Владелец продукта выбрал другой
> контракт: после одного явного предупреждения setup выдаёт wildcard всему
> управляемому серверу Yandex KIT, чтобы новые инструменты не создавали новые
> Allow. Текущий исполняемый контракт находится в
> [`approval-policies.md`](../skills/a1-yandex-kit-setup/references/approval-policies.md).
> Ниже сохранён исходный анализ более узкой альтернативы; он не является
> инструкцией для setup.
...
Исследование подтвердило синтаксис wildcard и показало его цену: он охватывает
опасные и будущие инструменты. Продукт принимает эту цену после одного явного
согласия пользователя. Setup применяет server-scoped wildcard там, где клиент
даёт проверяемый контракт, проводит через один ручной шаг при наличии
server-wide UI и заранее сообщает об отсутствии такой возможности в остальных
клиентах.
