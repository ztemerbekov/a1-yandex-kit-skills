...
> [!NOTE]
> **Заморожен — исторический документ.** Это снимок первоначального плана на 2026-07-27.
> План реализован; файл перенесён в `docs/history/` и как источник контекста хранится ради
> проектного «почему» (стратегия зеркалирования Shopify, рассмотренные альтернативы §4,
> лог фаз §6, DoD §11), а не как актуальная инструкция. Живой source of truth по API —
> `specs/kit-swagger.openapi.json`; актуальные конвенции — в `CLAUDE.md`, `docs/DEVELOPMENT.md`,
> `docs/TECHNICAL.md`, `docs/PUBLISHING.md`.
>
> Устаревшие места (не правятся, чтобы сохранить снимок как есть):
> - Репозиторий переименован в `a1-yandex-kit-ai-toolkit`.
> - §10.2 «не публиковать без решения по неймспейсу» — решено: `server.json` использует
>   `io.github.gistrec/…`, мейнтейнер Glama — `gistrec`.
> - Навыков сейчас 11 (6 сгенерированных доменных + 5 ручных), а не 6, как в §6.
> - Кураторских тулов 61, а не «~59» из §11.
> - `CHANGELOG.md` (упомянут в §3/§4.10/§9) не заводился.
> - Чекбоксы DoD в §11 оставлены пустыми, хотя работа выполнена и опубликована.
