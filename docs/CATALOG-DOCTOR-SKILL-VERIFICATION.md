...
`packages/mcp/src/scenarios/catalog-doctor-skill-scenario.ts` содержит
детерминированную reference model, fake MCP и tracer для аудита. Тесты
`catalog-doctor-skill-scenario.test.ts` и
`catalog-doctor-skill-structure-scenario.test.ts` покрывают многостраничные и
прерванные аудиты, fallback-чтения связей, группировку, медиа, коллекции, уровни
полноты, здоровый каталог и запрошенный владельцем мерчандайзинг.

`packages/mcp/src/scenarios/catalog-doctor-skill-fix-scenario.ts` содержит
reference model точных исправлений. `catalog-doctor-skill-fix-scenario.test.ts`
проверяет цену через read/write/re-read, сгруппированный запрос отсутствующего
источника без записи, сохранение соседних остатков и медиа, безвозвратное удаление
по точному глаголу, полные пакетные результаты, массивы из detail-read и
неоднозначные timeout/5xx без второй мутации.

Проверяются наблюдаемые вызовы, аргументы, покрытие, классификация, состояние и
отсутствие лишних записей, а не дословная формулировка ответа. Эти сценарии не
исполняют `SKILL.md` через LLM-хост; end-to-end соответствие скилла подтверждается
отдельной ручной приёмкой ниже.

## Матрица issues #10–#12
...
| Runtime `SKILL.md` маршрутизирует аудит в локальные scope references до первого MCP-вызова | `the catalog doctor progressively discloses every audit scope` | автоматизировано |
| Общий exact-write протокол имеет один канонический исходник и одинаковые локальные копии | `the shared exact-write protocol is generated identically into both skills`; `npm run gen` | автоматизировано |
