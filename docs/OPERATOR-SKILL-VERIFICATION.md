# Проверка скилла `a1-yandex-kit-operator`

## Контракт автоматизированных сценариев

`packages/mcp/src/scenarios/operator-skill-scenario.ts` содержит детерминированную
reference model и переиспользуемый fake MCP. Подготовленные данные покрывают заказы,
SKU, продукты, промо и вебхуки; fake MCP поддерживает пагинацию, связи промо,
подготовленные ошибки записи, журнал имён и аргументов инструментов, изменение
состояния и итоговый отчёт.

`operator-skill-scenario.test.ts` проверяет read-only разборы и частичные чтения, а
`operator-skill-write-scenario.test.ts` — точные подтверждения, отмены, цены, остатки,
промо, вебхуки, неоднозначные команды, timeout/5xx и несовпадающее проверочное чтение.
Проверяются наблюдаемые вызовы, аргументы, состояние и итоговый отчёт, а не дословная
формулировка ответа.

Эти сценарии не исполняют `SKILL.md` через LLM-хост. End-to-end соответствие скилла
подтверждается отдельной ручной приёмкой ниже.
...
| Факты оплаты, отмены/возврата, доставки и итоговый отчёт только после попытки всех независимых секций | `the operator completes every independent review scope before producing the report`; фикстуры содержат ожидающее подтверждения, просроченную доставку и возврат | автоматизировано |
...
1. «Как дела в магазине?» при ошибке второй страницы заказов — все независимые разделы
   предприняты до формирования ответа, ответ содержит точное частичное покрытие,
   записей нет.
