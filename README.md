...
7 [Agent Skills](https://agentskills.io/) для Codex, Claude Code, Cursor и других совместимых агентов. Шесть доменных навыков самодостаточны: спека забандлена (`data/kit_v1.json.gz`), поиск по докам и офлайн-валидация тел запросов запускаются голым Node.js >= 20, без `npm install` и без сети. Отдельный операторский навык оркестрирует их для read-only разбора магазина:
...
- `a1-yandex-kit-operator` — текущий статус магазина: объединяет заказы с быстрыми критическими сигналами опубликованных SKU, промо и вебхуков. Глубокий аудит каталога передаёт `a1-yandex-kit-catalog-doctor`; запросы «Как дела в магазине?» и «Что срочного?» не изменяют данные.
