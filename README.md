...
- **Надёжная база** — типизированный клиент, 309 тестов без сети, CI на Node 20/22/24, сгенерированный код проверяется на дрейф.
...
Скилл записывает команду `npx -y mcp-yandex-kit@latest`, поэтому MCP-сервер проверяет свежую версию при запуске. Отдельная команда обновления сервера не нужна. Повторный запуск настройки сохраняет текущий токен по ответу пользователя и нормализует команду на `@latest`.
