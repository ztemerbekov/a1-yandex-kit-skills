...
7 [Agent Skills](https://agentskills.io/) для Codex, Claude Code, Cursor и других совместимых агентов. Шесть доменных навыков самодостаточны: спека забандлена (`data/kit_v1.json.gz`), поиск по докам и офлайн-валидация тел запросов запускаются голым Node.js >= 20, без `npm install` и без сети. Отдельный операторский навык оркестрирует их для разбора магазина и точно заданных владельцем операций:
