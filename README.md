...
- `a1-yandex-kit-operator` — текущий статус магазина: читает все страницы заказов, выделяет новые и ожидающие подтверждения заказы, фактические риски оплаты, отмены/возврата и просроченной доставки. Запросы «Как дела в магазине?» и «Что срочного?» не изменяют данные.
