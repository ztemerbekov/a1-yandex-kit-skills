...
![Демо: настоящая MCP-сессия mcp-yandex-kit — магазин, каталог, заказы и мета-инструменты](docs/demo.gif)
...
- **Устойчивый клиент** — встроенный лимитер 3 rps (token bucket), ретраи с бэкоффом на сетевых ошибках, 5xx, 429 и `LIMIT_EXCEEDED`, автопагинация, корректные content-type (`merge-patch+json`, `multipart/form-data`) там, где их требует API.
