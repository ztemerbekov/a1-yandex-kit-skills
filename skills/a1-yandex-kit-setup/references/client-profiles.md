...
|---|---|---|---|---|---|
| Claude Code | `claude-code` | JSON `mcpServers` in `~/.claude.json` | `claude mcp --help`; confirm user scope | captured `claude mcp list` | Start a new Claude Code session |
...
| Kimi Code | `kimi` | JSON `mcpServers` in `$KIMI_CODE_HOME/mcp.json` or `~/.kimi-code/mcp.json` | `kimi --help` plus the current official MCP page | structural parse and exact entry; `/mcp` is interactive | Start a new Kimi Code session |
...
| OpenClaw | `openclaw` | nested JSON `mcp.servers` in `~/.openclaw/openclaw.json` | `openclaw mcp --help` | captured `openclaw mcp doctor yandex-kit --probe` | Run the documented MCP reload command shown by local help, or restart OpenClaw |
