...
// src/skill-src/validate.src.mjs
var import_ajv = __toESM(require_ajv(), 1);
import { readFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { gunzipSync } from "node:zlib";
var AjvClass = typeof import_ajv.default === "function" ? import_ajv.default : import_ajv.default.default;
var DATA_PATH = fileURLToPath(new URL("../data/kit_v1.json.gz", import.meta.url));
...
for (let i = 0; i < argv.length; i++) {
  ...
  if (arg === "--operation") {
    ...
    bodyText = argv[++i];
    if (bodyText === void 0) fail("--body requires a JSON string\n\n" + USAGE);
...
var spec = JSON.parse(gunzipSync(readFileSync(DATA_PATH)).toString("utf8"));
var operation = null;
...
if (operation === null) {
  ...
  const similar = known.map((id) => ({ id, score: bigramSimilarity(target, id.toLowerCase()) })).sort((a, b) => b.score - a.score || (a.id < b.id ? -1 : 1)).slice(0, 5).map((entry) => entry.id);
...
var contentTypes = Object.keys(operation.requestBody?.content ?? {});
var contentType = contentTypes[0] ?? null;
var rawSchema = contentType ? operation.requestBody.content[contentType].schema ?? null : null;
var bodyProvided = bodyText !== null && bodyText.trim() !== "";
if (rawSchema === null) {
  if (bodyProvided) {
...
var rootSchema = typeof rawSchema.$ref === "string" ? { $ref: rawSchema.$ref } : transformNullable(rawSchema);
var ajv = new AjvClass({ strict: false, allErrors: true, validateFormats: false });
...
var errors = (validate.errors ?? []).map(
