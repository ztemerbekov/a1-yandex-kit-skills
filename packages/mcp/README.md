...
Внутри — **61 MCP-инструмент**: кураторские тулы на повседневные операции (65 из 151 операции API) плюс мета-трио (`search_operations`, `get_operation_schema`, `kit_request`), через которое доступна **каждая из 151 операции**. Под капотом — клиент [`yandex-kit-core`](https://www.npmjs.com/package/yandex-kit-core): лимитер 3 rps, ретраи с бэкоффом для чтений (write-запросы не повторяются автоматически), ajv-валидация тел write-запросов **до** отправки в сеть.
...
- **3 запроса в секунду на магазин** — жёсткий лимит API. При превышении приходит `LIMIT_EXCEEDED` с **HTTP 400** (не 429) — клиент сам троттлит и для чтений ретраит с бэкоффом; write-запросы не повторяются автоматически.
