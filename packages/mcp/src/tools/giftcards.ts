...
export function registerGiftCardTools(server: McpServer, client: KitClient): void {
  server.registerTool(
    ...
    async ({ page, per_page, all, status, purchased_date_from, purchased_date_to }) => {
      const filters = { status, purchased_date_from, purchased_date_to };
      try {
        if (all) return ok(await client.listAll("GetGiftCards", { query: filters }));
        return ok(
  ...
  server.registerTool(
    ...
    async ({ id }) => {
      try {
        return ok(await client.call("GetGiftCardById", { pathParams: { gift_card_id: id } }));
      } catch (e) {
        return fail(e);
      }
    },
